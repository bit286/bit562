...
/**************************************************************************************
 * Managing database connections is a critical feature of any program
 * interacting with a database.  Having an object to perform the basic
 * house cleaning of connection and execution makes for good clarity of code
 * throughout programs using the database.  Having a connection object
 * allows for connection to multiple databases at the same time, each
 * database gets its own database manager.
 *
 * A sample dsn string for a mysql database looks like the following:
 *    $db_dsn = "mysql:host={$db_host};dbname={$db_database}";
 * where $db_host is often "localhost" and database name is the local
 * schema.
 * The $db_host, $db_database, $db_username, and $db_password are typically
 * stored in a db_login.php file which is imported prior to constructing
 * the DBManager
 */
class DBManager {
     ...
     /*+ TODO::location=DBManager.php::task=When fail to open, pass control to an included file which displays an acceptable user message.  See pp 76-77 in text.::author=Ed Anderson*/
     ...
     public function test( $description ) {
         ...
         if ( $this->positiveTest == true)
         {
             ...
             $testSQL = "INSERT INTO test SET description = '".
                 $description."'";
