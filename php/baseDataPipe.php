...
class BaseDataPipe {
	...
	protected function update() {
		...
		for ( $i=0, $len=count($this->map); $i<$len; $i+=1 ) {
			...
			if ( $this->map[$i]->getDataType() == "alpha" ) {
		...
		switch ( $this->queryType ) {
			...
			case "select" :
				while ($row = $result->fetch(PDO::FETCH_ASSOC)) {					
					...
					for ( $i=0; $i<count($this->map)-$offset;$i+=1 ) {
						$XML .= "<field name=\"".$this->map[$i]->get('browser').
						           "\">".$this->db->deScrub($row[$i]);
						$XML .= "</field>";
					}

					if ( strlen($record[$i]) > 0 ) {
						$XML .= "<field name=\"commentPresent\">".$this->db->deScrub($row[$i]);
						$XML .= "</field>";
					}
					$XML .= "</row>";
